#!/usr/bin/env bash
# Hamilton's fork from: https://github.com/roblib/scripts/blob/master/bash/dvdripper.sh
# Robinson stores all their islandora modules in .../sites/all/islandora_modules/
# so they pass our islandora sites all directory to this script as a parameter
...
# hamilton: ./update_islandora_head.sh /var/www/drupal/sites/all
...
echo $tempDir
(cp -r $1/modules $tempDir) || { echo "ERROR Failed to backup islandora_modules directory"; exit 1; }
...
for d in $1/modules/islandora*/ ; do
    cd "$d"
    pwd
 git fetch
...
# Might not be a git-able library
# if not, download from https://github.com/Islandora/tuque/
